...
class App extends Component {
  ...
  signedInChanged = (signedIn) => {
    ...
    if (this.state.signedIn) {
      document.body.className -= ' ' + 'intro-image';
      this.load();
    } else {
      //get body tag
      document.body.className += ' ' + 'intro-image';
  ...
  // preAuthCardForPurchase(cardId, terminalId) {
  //   if (cardId.state.isCardActive && terminalId.location != constricted)  {
  //        return true;
  //   }
  // }
  // handleAutomaticExpense = (event, ) => { event.place, event.amount, }
  ...
  renderBody() {
    if (this.state.processing) return <LoadingBar />;
      ...
      return (
        <div className="content col col-md-6 align-self-center offset-md-3 col-lg-16">
          {this.renderExpenses()}
        </div>
      );
  }
            ...
            <section className="mdc-card__primary">
              ...
              <h1 className="mdc-card__title mdc-card__title--large">
                <span className="left-showcase">
                  <span className="green"><sup><span className="big-number">{this.state.currentMonthIn}</span> in</sup></span>&frasl;<span className="red"><sub><span className="big-number-down">{this.state.currentMonth}</span> out</sub></span>
                </span>
